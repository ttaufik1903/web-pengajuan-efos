...
import com.rabbai.serviceparefos.models.Par_template_dokumen;
...
public interface Par_template_dokumenRepository extends JpaRepository<Par_template_dokumen, Integer> {
    ...
    @Query(value = "SELECT * FROM par_template_dokumen WHERE \"id\"=:id ORDER BY id ASC", nativeQuery = true)
    Optional<Par_template_dokumen> findByid(@Param("id") Integer id);
    
    @Query(value = "SELECT * FROM par_template_dokumen WHERE \"kategori\"=:kategori ORDER BY id ASC", nativeQuery = true)
    List<Par_template_dokumen> findByIdTemplate(@Param("kategori") String kategori);
