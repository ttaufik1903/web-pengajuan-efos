...
import com.rabbai.serviceparefos.models.Par_cabang;
...
public interface Par_cabangRepository extends JpaRepository<Par_cabang, String> {
    ...
    @Query(value = "SELECT *   FROM par_cabang where  \"kota\"=:kota and inisial not in ('KPS','','820')" , nativeQuery = true)
    List<Par_cabang> findCabangByWilayah(@Param("kota") String id);
    
    @Query(value = "SELECT DISTINCT kota FROM par_cabang order by kota asc" , nativeQuery = true)
    List<String> findDistictKabupataen();
    ...
    @Query(
            value = "SELECT * FROM par_cabang ORDER BY \"id\" ASC",
            countQuery = "SELECT count(*) FROM par_cabang",
            nativeQuery = true)
    Page<Par_cabang> findAllWithPagination(Pageable pageable);

    @Query(
            value = "SELECT * FROM par_cabang WHERE cabang ~* :keyword" +
                    " OR jenis_cab ~* :keyword OR id ~* :keyword OR kota ~* :keyword" +
                    " ORDER BY id ASC",
            countQuery = "SELECT count(*) FROM par_cabang WHERE cabang ~* :keyword" +
                    " jenis_cab ~* :keyword OR id ~* :keyword OR kota ~* :keyword",
            nativeQuery = true)
