...
public class UsulanController {
	...
	private static Logger LOGGER = Logger.getLogger(UsulanController.class.getName());

	@PostMapping("/usulan/calondebitur/list")
	@ResponseBody
...
//            TimeUnit.SECONDS.sleep(50);
	...
	@PostMapping("/usulan/calondebitur/inquiry")
	@ResponseBody
	@Transactional
	...
	@PostMapping("/usulan/calondebitur/inquiryusulanbyid")
	@ResponseBody
...
//				LOGGER.info("!! LOAN (INQUIRY LOAN BY ID) !! " + "@ " + objectMapper.writeValueAsString(usul) + " \n");
				...
				response.put("plan_type", loan.getType_plan());
				response.put("kode_cabang_pengusul", loan.getId_cab());
				response.put("nama_cabang_pengusul", loan.getCabang().getCabang());
			} else {
				System.out.println("##### TIDAK ADA ##### ");
				response.put("kode", "10");
	...
	@GetMapping("/usulan/calondebitur/deleteloanbyid/{id_loan}")
	@Transactional
...
//            debiturRepository.save(instansi);
	...
	@GetMapping("/usulan/calondebitur/delete/{ktp}")
	@Transactional
...
//            debiturRepository.save(instansi);
	...
	@Transactional
	@PostMapping("/usulan/calondebitur/submit")
	@ResponseBody
	...
	@PostMapping("/usulan/calondebitur/review")
	@ResponseBody
	...
	@PostMapping("/usulan/notisi/reject")
	@ResponseBody
	public ResponseEntity<Map<String, Object>> setNotisiReject(@Valid @RequestBody Map<String, String> param,
		...
		try {
			...
			if (usul != null) {
					...
					Loan data_loan = loanRepository.findByIdLoan(usul.getId_loan());
					data_loan.setStatus("12");
					data_loan.setReview_desc(keterangan);
					data_loan.setReview_date(LocalDateTime.now());
					data_loan.setReview_by(user_id);
					data_loan.setReview_nama(user_nama);
					loanRepository.save(data_loan);
	...
	public ResponseEntity<Map<String, Object>> setuploadsp4(@Valid @RequestBody Map<String, String> param,
		...
		try {
			Usulan usulan = usulanRepository.findByIdLoan(id_usulan);
		...
		LOGGER.info("\n ************** RESPONSE TO FRONTEND /usulan/notisi/uploadsp4 ************** " + "@ "
				+ objectMapper.writeValueAsString(response) + " \n");
		return new ResponseEntity<>(response, HttpStatus.OK);
	}
	
	@PostMapping("/usulan/notisi/review")
	@ResponseBody
	...
	@PostMapping("/usulan/notisi/approve1")
	@ResponseBody
...
//					} else 
	...
	@Transactional
	@PostMapping("/usulan/notisi/approve2")
	@ResponseBody
...
//					} else 
	...
	@Transactional
	@PostMapping("/usulan/notisi/approve3")
	@ResponseBody
	...
	@PostMapping("/usulan/notisi/forward")
	@ResponseBody
