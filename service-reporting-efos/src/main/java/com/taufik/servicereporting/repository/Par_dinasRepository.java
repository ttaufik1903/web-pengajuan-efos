...
import com.taufik.servicereporting.models.Par_dinas;
...
public interface Par_dinasRepository extends JpaRepository<Par_dinas, String> {
    ...
    @Query(
            value = "SELECT * FROM par_dinas ORDER BY \"id\" ASC",
            countQuery = "SELECT count(*) FROM par_dinas",
            nativeQuery = true)
    Page<Par_dinas> findAllWithPagination(Pageable pageable);

    @Query(
            value = "SELECT * FROM par_dinas WHERE name ~* :keyword" +
                    " OR CAST(id AS text) ~* :keyword" +
                    " ORDER BY id ASC",
            countQuery = "SELECT * FROM par_dinas WHERE name ~* :keyword" +
                    " OR CAST(id AS text) ~* :keyword",
            nativeQuery = true)
    Page<Par_dinas> findKeywordWithPagination(Pageable pageable, @Param("keyword") String keyword);


    @Query(value = "SELECT MAX(\"id\") as \"id\" FROM par_dinas",nativeQuery = true)
    Integer getMaxId();
    
    @Query(value = "SELECT count(*) FROM par_dinas", nativeQuery = true)
    Integer getCount();
