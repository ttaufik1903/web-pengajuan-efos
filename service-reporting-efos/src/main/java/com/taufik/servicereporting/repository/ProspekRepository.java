...
public interface ProspekRepository extends JpaRepository<Data_prospek, Integer> {
	...
	@Query(value = "SELECT * FROM data_prospek WHERE DATE(datepost_open) BETWEEN :tgl1 and :tgl2 and jenis_pembiayaan=:jenis_pembiayaan", nativeQuery = true)
	List<Data_prospek> findDataProspekForReportAll(LocalDate tgl1 , LocalDate tgl2, String jenis_pembiayaan);

	@Query(value = "SELECT * FROM data_prospek WHERE DATE(datepost_open) BETWEEN :tgl1 and :tgl2 and jenis_pembiayaan=:jenis_pembiayaan and cab=:cab", nativeQuery = true)
	List<Data_prospek> findDataProspekForReport(LocalDate tgl1 , LocalDate tgl2, String jenis_pembiayaan, Integer cab);
